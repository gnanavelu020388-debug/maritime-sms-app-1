  ...
  const lines = [
    ...
    ...(invoice.lineItems?.length
      ? invoice.lineItems.map(
          (li) =>
            `  - ${li.description}: ${formatCurrency(li.amount, invoice.currency)}`,
        )
      : ["  (none itemized)"]),
    ``,
...
export function BillingView({ caps }: { caps: Capabilities }) {
  ...
  // Contract Lifecycle Monitor mirrors Central Transaction Vault's actual
  // rendered height so it neither leaves blank space (Vault shorter) nor
  // gets clipped oddly (Vault taller) — a plain CSS grid can't do this on
  // its own because neither card has a fixed height to stretch against;
  // the row would just grow to fit whichever card's content is tallest,
  // leaving blank space under the shorter one. Only applied at the `lg`
  // breakpoint where the two cards actually sit side by side; below that
  // they stack, so each keeps its own natural height instead.
  const vaultRef = useRef<HTMLDivElement>(null);
  const [vaultHeight, setVaultHeight] = useState<number | null>(null);
  const [sideBySide, setSideBySide] = useState(false);

  useEffect(() => {
    const mq = window.matchMedia("(min-width: 1024px)");
    const updateSideBySide = () => setSideBySide(mq.matches);
    updateSideBySide();
    mq.addEventListener("change", updateSideBySide);
    return () => mq.removeEventListener("change", updateSideBySide);
  }, []);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      setVaultHeight(entry.contentRect.height);
    });
    observer.observe(el);
    return () => observer.disconnect();
  }, []);

  const overLimitCount = tenants.filter(
    (t) =>
  ...
  const invoiceColumns: Column<Invoice>[] = [
    ...
    },
    {
      key: "company",
      header: "Company",
      sortValue: (i) => i.company,
      render: (i) => (
        <div>
          <p className="text-sm font-medium text-ink-800 dark:text-ink-100">
            {i.company}
          </p>
          <p className="text-xs text-ink-500">{i.tenantId}</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (i) => (
        <span className="text-xs text-ink-500">{relativeTime(i.issued)}</span>
      ),
    },
    {
      key: "status",
  ...
  const expiring = tenants
    .map((t) => ({ tenant: t, days: daysUntil(t.contractExpires) }))
    .sort((a, b) => a.days - b.days);

  const [savingTiers, setSavingTiers] = useState(false);

  // The name last confirmed to exist in the DB for each tier row, tracked
  // separately from the live-edited tierConfigs so a rename in progress
  // doesn't lose track of which row to PUT to. Resynced whenever tierConfigs
  // reflects real saved/hydrated state (i.e. not while an edit is pending).
  const [savedNames, setSavedNames] = useState(() => tierConfigs.map((t) => t.name));
  useEffect(() => {
    if (!dirty) setSavedNames(tierConfigs.map((t) => t.name));
  }, [tierConfigs, dirty]);

  const updateTier = (idx: number, patch: Partial<TierConfig>) => {
    if (!caps.billingEdit) return;
    // Local edit only — existing tenants keep their current limits/name
    // until "Save tiers" persists this. The one exception is a rename,
    // which — once saved — cascades to every tenant already on that plan
    // (see saveTiers), since the name is the key tenants.plan points at.
    dispatch({ type: "TIER_CONFIG_UPDATE", index: idx, patch });
    setDirty(true);
  };

  const saveTiers = async () => {
    const trimmedNames = tierConfigs.map((t) => t.name.trim());
    if (trimmedNames.some((n) => !n)) {
      toast({ tone: "danger", title: "Cannot save", message: "Every plan needs a name." });
      return;
    }
    if (new Set(trimmedNames.map((n) => n.toLowerCase())).size !== trimmedNames.length) {
      toast({ tone: "danger", title: "Cannot save", message: "Plan names must be unique." });
      return;
    }

    setSavingTiers(true);
    try {
      const renames: { before: string; after: string }[] = [];
      await Promise.all(
        tierConfigs.map((t, i) => {
          const oldName = savedNames[i];
          const newName = t.name.trim();
          if (newName !== oldName) renames.push({ before: oldName, after: newName });
          return apiUpdateTierConfig(oldName, {
            monthly: t.monthly,
            annual: t.annual,
            vessels: t.vessels,
            storage_gb: t.storageGb,
            seats: t.seats,
            new_name: newName !== oldName ? newName : undefined,
          });
        }),
      );
      await logAudit({
        ...
        category: "billing",
        action: renames.length > 0 ? "Tier configuration saved (plan renamed)" : "Tier configuration saved",
        target: "Platform-wide",
        severity: renames.length > 0 ? "warning" : "info",
        before: renames.length > 0 ? { renamed: renames } : undefined,
      ...
      setSavedNames(tierConfigs.map((t) => t.name.trim()));
      setDirty(false);
      // A rename cascades to every tenant on that plan server-side — pull
      // fresh tenant data so the Ledger/Monitoring reflect it immediately
      // instead of only after a manual reload.
      if (renames.length > 0) await hydrateAllTenants(dispatch);
      toast({
        tone: "success",
        title: "Tier configuration saved",
        message:
          renames.length > 0
            ? `Renamed ${renames.map((r) => `"${r.before}" → "${r.after}"`).join(", ")}. Existing tenants updated.`
            : "Pricing and quota thresholds persisted for all future plan assignments.",
      });
    } catch (err) {
      toast({
        tone: "danger",
        title: "Failed to save tier configuration",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-ink-500 dark:text-ink-400">
          SaaS tier constructor, transaction vault & contract lifecycle.
        </p>
      </div>
      ...
      <Card
        ...
        subtitle="Sets default pricing & quotas applied to new plan assignments (Upgrade or provisioning)"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-ink-100 dark:divide-ink-800">
              {tierConfigs.map((t, i) => (
                <tr key={i} className="bg-white dark:bg-ink-900">
                  <td className="px-3 py-1">
                    <input
                      ...
                      className="input w-36 py-1 text-sm font-bold text-ink-900 disabled:opacity-60 dark:text-white"
                    />
                  </td>
                  <td className="px-3 py-1">
                    <input
                      ...
                      disabled={!caps.billingEdit}
                      onChange={(e) =>
                        updateTier(i, { monthly: +e.target.value })
                      }
                      className="input w-24 py-1 text-sm disabled:opacity-60"
                    />
                  </td>
                  <td className="px-3 py-1">
                    <input
                      ...
                      disabled={!caps.billingEdit}
                      onChange={(e) =>
                        updateTier(i, { annual: +e.target.value })
                      }
                      className="input w-24 py-1 text-sm disabled:opacity-60"
                    />
                  </td>
                  <td className="px-3 py-1">
                    <input
                      ...
                      disabled={!caps.billingEdit}
                      onChange={(e) =>
                        updateTier(i, { vessels: +e.target.value })
                      }
                  ...
                  <td className="px-3 py-1">
                    <input
                      ...
                      disabled={!caps.billingEdit}
                      onChange={(e) =>
                        updateTier(i, { storageGb: +e.target.value })
                      }
                      className="input w-24 py-1 text-sm disabled:opacity-60"
                    />
                  </td>
                  <td className="px-3 py-1">
                    <input
                      ...
                      disabled={!caps.billingEdit}
                      onChange={(e) =>
                        updateTier(i, { seats: +e.target.value })
                      }
                      className="input w-20 py-1 text-sm disabled:opacity-60"
                    />
                  </td>
          ...
          {!caps.billingEdit && (
            <p className="mt-2 rounded-lg bg-ink-50 p-2 text-xs text-ink-500 dark:bg-ink-800/50 dark:text-ink-400">
              Tier construction is read-only for your role. Editing is
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        {/* Transaction Vault */}
        {/* self-start: without it, CSS Grid's default stretch would inflate
            this wrapper to match Contract Lifecycle's natural (unclamped)
            height before the ResizeObserver below ever gets a true reading,
            which is what caused the two cards to fight over height. */}
        <div ref={vaultRef} className="lg:col-span-2 self-start">
          <Card
          ...
          >
            <div className="mb-3 flex items-start gap-2 rounded-lg bg-primary-50 p-3 text-xs text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">
              ...
              <span>
                Recurring monthly invoices generate{" "}
                <strong>automatically</strong> on each tenant's renewal date
                based on active vessel count. Use{" "}
                <strong>Generate invoice</strong> to create ad-hoc invoices
                manually.
              </span>
            </div>
            <DataTable
              ...
              pageSize={8}
              searchable
              searchPlaceholder="Search invoices, companies…"
              searchFn={(i, q) =>
        ...
        >
          <div className="h-full min-h-0">
            <Card
              title="Contract Lifecycle Monitor"
              subtitle="Renewals within 90-day window · configurable recipient emails"
            ...
            >
              <div className="space-y-2.5">
                {expiring.map(({ tenant, days }) => {
                    const tone =
                      days < 30 ? "danger" : days < 60 ? "warning" : "neutral";
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-lg border p-3 ${
                          tone === "danger"
                            ? "border-danger-200 bg-danger-50/40 dark:border-danger-900/50 dark:bg-danger-900/10"
                            : tone === "warning"
                              ? "border-warning-200 bg-warning-50/40 dark:border-warning-900/50 dark:bg-warning-900/10"
                          ...
                          <div className="mt-1 flex items-center gap-1.5">
                            ...
                            >
                              <input
                                type="email"
                                value={
                                  isEditing ? pendingEmail : tenant.contactEmail
                                }
                                readOnly={isLocked}
                                onChange={(e) =>
                                ...
                                className={`input py-1.5 text-xs ${
                                  isLocked
                                    ? "bg-ink-50/70 dark:bg-ink-800/40"
                              ...
                              <span className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-ink-400">
                                {isLocked ? (
                                  ...
                                  <Unlock className="h-3.5 w-3.5 text-primary-500" />
                                )}
                              </span>
                            </div>
                            ...
                            <button
                              onClick={async () => {
                                if (isEditing) {
                                  ...
                                  dispatch({
                                    type: "TENANT_UPDATE",
                                    id: tenant.id,
                                    patch: { contactEmail: pendingEmail },
                                  });
                                  ...
                                  await logAudit({
                                    ...
                                    actorEmail: user?.email ?? "unknown",
                                    category: "billing",
                                    action: `Renewal recipient email updated: ${tenant.company}`,
                                  ...
                                  toast({
                                    tone: "success",
                                    title: "Recipient email saved",
                                    message: `${tenant.company} → ${pendingEmail}`,
                                  });
                              ...
                              title={
                                isEditing ? "Save email" : "Unlock to edit"
                              }
                            >
                              {isEditing ? (
                          ...
                          <button
                            onClick={async () => {
                              const newExpiry = new Date(
                                new Date(tenant.contractExpires).getTime() +
                                  30 * 86400000,
                              ).toISOString();

                              try {
                                await apiUpdateTenant(tenant.id, {
                                  contract_expires: newExpiry,
                                });
                              } catch (err) {
                                toast({
                                  tone: "danger",
                                  title: "Failed to grant extension",
                                  message: (err as Error).message,
                                });
                                return;
                              }
                              ...
                              dispatch({
                                type: "TENANT_UPDATE",
                                id: tenant.id,
                                patch: { contractExpires: newExpiry },
                              });
                              ...
                              await logAudit({
                                ...
                                actorEmail: user?.email ?? "unknown",
                                category: "billing",
                                action: `Temporary extension granted: ${tenant.company}`,
                                target: tenant.company,
                                severity: "warning",
                                before: {
                                  contract_expires: tenant.contractExpires,
                                },
                                after: {
                                  contract_expires: newExpiry,
                                },
                              ...
                              toast({
                                ...
                                message: `${tenant.company} renewal pushed back 30 days.`,
                              });
                            }}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <RevCard
          ...
          sub="Monthly recurring revenue (archived excluded)"
        />
        <RevCard
        ...
        <RevCard
          ...
          sub="Requires collection action"
          tone="warning"
        />
      </div>
            ...
            <>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    await apiSendBillingReminder(
                      emailPreview.id,
                      `Renewal notice for ${emailPreview.company}, expires ${new Date(emailPreview.contractExpires).toLocaleDateString("en-GB")}.`,
                    );
                    await logAudit({
                      tenantId: emailPreview.id,
                      actorEmail: user?.email ?? "unknown",
                      category: "billing",
                      action: `Renewal reminder sent: ${emailPreview.company}`,
                      target: emailPreview.contactEmail,
                      severity: "info",
                    ...
                    toast({
                      ...
                      message: `Recorded for ${emailPreview.company} — your email client opened with the draft ready to send.`,
                    });
                    setEmailPreview(null);
                  } catch (err) {
                    toast({
              ...
              >
                {emailSending ? (
                  ...
                  <>
                    <Send className="h-4 w-4" /> Send Reminder Email Now
                  </>
                )}
              </button>
            </>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-[60px_1fr] gap-x-3 gap-y-2 rounded-xl border border-ink-200 bg-ink-50/60 p-4 text-sm dark:border-ink-800 dark:bg-ink-800/40">
              ...
              <span className="font-mono text-ink-800 dark:text-ink-100">
                {emailPreview.contactEmail}
              </span>
              <span className="font-semibold text-ink-500">Subject:</span>
              <span className="font-semibold text-ink-800 dark:text-ink-100">
                Action Required: Subscription Renewal Notice —{" "}
                {emailPreview.company}
              </span>
            </div>
            <div>
            ...
            <div className="rounded-lg bg-primary-50 p-2.5 text-xs text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">
              ...
              Plan: {emailPreview.plan} · Expiry:{" "}
              {new Date(emailPreview.contractExpires).toLocaleDateString(
                "en-GB",
              )}{" "}
              · MRR: {formatCurrency(emailPreview.monthlyRevenue)}/mo
            </div>
          </div>
        </Modal>
      )}
            ...
            <>
              ...
              {deleteStep === 1 && (
                <button onClick={() => setDeleteStep(2)} className="btn-danger">
                  <Trash2 className="h-4 w-4" /> Yes, continue
                </button>
              )}
              {deleteStep === 2 && (
                <button
                  onClick={async () => {
                    try {
                      await apiUpdateTenant(deleteFor.id, {
                        contact_email: "",
                      });
                    } catch (err) {
                      toast({
                    ...
                    await logAudit({
                      tenantId: deleteFor.id,
                      actorEmail: user?.email ?? "unknown",
                      category: "billing",
                      action: `Renewal configuration removed: ${deleteFor.company}`,
                      target: deleteFor.company,
                      severity: "critical",
                      before: { contact_email: deleteFor.contactEmail },
                      after: { contact_email: "" },
                    });
                    toast({
                      tone: "danger",
                      title: "Configuration removed",
                      message: `Renewal configuration for ${deleteFor.company} has been wiped.`,
                    });
                    setDeleteFor(null);
                    setDeleteTyped("");
                    setDeleteStep(1);
          ...
          {deleteStep === 1 ? (
            <div className="space-y-3">
              <div className="flex items-start gap-2 rounded-lg bg-danger-50 p-3 text-sm text-danger-800 dark:bg-danger-900/20 dark:text-danger-300">
                ...
                <span>
                  Are you sure you want to remove the renewal configuration for{" "}
                  <strong>{deleteFor.company}</strong>? This will clear the
                  recipient email and halt automated renewal reminders.
                </span>
              </div>
              <div className="rounded-lg border border-ink-200/70 p-3 text-xs text-ink-500 dark:border-ink-800">
                ...
                <div className="mt-1.5 flex items-center justify-between">
                  ...
                  <span className="text-ink-700 dark:text-ink-200">
                    {deleteFor.plan} ·{" "}
                    {new Date(deleteFor.contractExpires).toLocaleDateString(
                      "en-GB",
                    )}
                  </span>
                </div>
            ...
            <div className="space-y-4">
              <div className="rounded-lg bg-danger-50 p-3 text-sm text-danger-800 dark:bg-danger-900/20 dark:text-danger-300">
                ...
                <strong>Tier 3 — Final safeguard:</strong> Type{" "}
                <span className="font-mono font-bold text-danger-600 dark:text-danger-400">
                  DELETE
                </span>{" "}
                below to permanently remove the renewal configuration for{" "}
                <strong>{deleteFor.company}</strong>.
              </div>
              <div>
                ...
                {deleteTyped && (
                  <p
                    className={`mt-1.5 text-xs ${deleteTyped.trim().toUpperCase() === "DELETE" ? "text-success-600 dark:text-success-400" : "text-danger-600 dark:text-danger-400"}`}
                  >
                    {deleteTyped.trim().toUpperCase() === "DELETE" ? (
                      <>
                        <CheckCircle2 className="mr-1 inline h-3 w-3" />
                        Confirmed — removal armed.
                      </>
                    ) : (
                      <>
                        <AlertTriangle className="mr-1 inline h-3 w-3" />
                        Type the full word DELETE to proceed.
                      </>
                    )}
                  </p>
                )}
        ...
        <GenerateInvoiceModal
          ...
          onSubmit={async (draft) => {
            ...
            await logAudit({
              ...
              actorEmail: user?.email ?? "unknown",
              category: "billing",
              action: `Invoice generated: ${inv.id}`,
            ...
            toast({
              ...
              message: `${inv.id} created for ${inv.company}.`,
            });
            setGenOpen(false);
          }}
        />
          ...
          <div>
            ...
            {lineTotal > 0 && !amount && (
              <p className="mt-1 text-xs text-ink-500">
                Auto-filled from line items: {formatCurrency(lineTotal)}
              </p>
            )}
          </div>
          <div>
...
function buildRenewalEmail(t: Tenant): string {
  const expiry = new Date(t.contractExpires).toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });
